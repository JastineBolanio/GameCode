 ...
 * 
 * Description:
 *   - User Profile page for SkillForge platform
 *   - Features:
    ...
    try {
        ...
        if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $targetFile)) {
                ...
                if (!empty($oldPicture) && $oldPicture !== 'assets/images/Avatar.gif' && file_exists($oldPicture)) {
                    @unlink($oldPicture);
                }
...
<div class="profile-banner-wrapper">
    ...
    <div class="profile-avatar-container">
        <img src="<?php echo !empty($currentUser['profile_picture']) ? htmlspecialchars($currentUser['profile_picture']) : '/assets/images/Avatar.gif'; ?>" 
             ...
             id="avatarPreview"
             onerror="this.onerror=null; this.src='/assets/images/Avatar.gif';">
    ...
    <div class="container py-4">
        ...
        <div class="row">
            <!-- Main Content -->
             <link rel="stylesheet" href="assets/css/ProfileV2.css">
            <div class="col-lg-8">
                <!-- Progress Ring Section -->
                <div class="card mb-4 gaming-sync-panel border-cyan">
  <div class="card-body">
    
    <div class="d-flex justify-content-between align-items-center mb-4 core-sync-header">
      <h5 class="card-title mb-0 gaming-panel-title">
        <i class="fas fa-microchip icon-cyan"></i> SYSTEM SYNC STATUS
      </h5>
    ...
    <div class="progress-ring-container telemetry-core-wrapper">
      
      <div class="progress-ring gaming-vector-ring">
        <?php 
        ...
        <svg class="progress-ring__circle" width="120" height="120" viewBox="0 0 120 120">
          <circle class="progress-ring__circle-bg terminal-circle-track" cx="60" cy="60" r="54" stroke-width="6" />
          <circle class="progress-ring__circle-fill terminal-circle-glowing-fill" 
                  cx="60" 
                  cy="60" 
                  r="54" 
                  stroke-width="6" 
        ...
        <div class="progress-ring__content terminal-ring-readout">
          <div class="progress-ring__percent dynamic-sync-value"><?php echo $stats['overall_progress']; ?>%</div>
          <div class="progress-ring__label dynamic-sync-label">SYNCED</div>
        </div>
      </div>
      
      <div class="progress-stats gaming-telemetry-grid">
        
        <div class="stat-item telemetry-cell">
          <div class="stat-value text-muted-data"><?php echo $stats['last_week_progress']; ?>%</div>
          <div class="stat-label cell-title">LAST WEEK</div>
        </div>
        
        <div class="stat-item telemetry-cell">
          <div class="stat-value text-muted-data"><?php echo $stats['last_month_progress']; ?>%</div>
          <div class="stat-label cell-title">LAST MONTH</div>
        </div>
        
        <div class="stat-item telemetry-cell accent-amber-cell">
          <div class="stat-value text-amber-data"><?php echo $stats['points']; ?></div>
          <div class="stat-label cell-title"><i class="fas fa-star"></i> NET_CREDITS</div>
        </div>
        
        <div class="stat-item telemetry-cell accent-matrix-cell">
          <div class="stat-value text-matrix-data">#<?php echo $stats['rank']; ?></div>
          <div class="stat-label cell-title"><i class="fas fa-trophy"></i> GLOBAL RANK</div>
        </div>
        
        <div class="stat-item telemetry-cell">
          <div class="stat-value text-cyan-data"><?php echo $stats['challenges_completed']; ?></div>
          <div class="stat-label cell-title">CHALLENGES</div>
        </div>
        
        <div class="stat-item telemetry-cell">
          <div class="stat-value text-cyan-data"><?php echo $stats['quizzes_passed']; ?></div>
          <div class="stat-label cell-title">QUIZZES</div>
        </div>
        
        <div class="stat-item telemetry-cell">
          <div class="stat-value text-purple-data"><?php echo $stats['mini_games_completed']; ?>/2</div>
          <div class="stat-label cell-title">MINI-GAMES</div>
        </div>
        
        <div class="stat-item telemetry-cell linear-progress-cell">
          <div class="d-flex justify-content-between align-items-center mb-1">
            <span class="cell-title">TUTORIAL MODULES</span>
            <span class="stat-value text-white-data"><?php echo $stats['tutorials_completed']; ?>/<?php echo $stats['total_topics']; ?></span>
          </div>
          <div class="progress stats-overview-progress terminal-linear-bar-frame">
            <div class="progress-bar stats-overview-progress-bar terminal-linear-bar-fill" style="width: <?php echo $stats['tutorial_progress_percentage']; ?>%"></div>
          </div>
        </div>
                ...
                <!-- About Me Section -->
                <div class="card mb-4 gaming-profile-card border-purple">
  <div class="card-body">
    
    <div class="d-flex justify-content-between align-items-center mb-3 terminal-profile-header">
      <h5 class="card-title mb-0 gaming-panel-title">
        <i class="fas fa-fingerprint icon-purple"></i> OPERATIVE INTEL
      </h5>
      <button class="btn btn-sm btn-gaming-edit" data-bs-toggle="modal" data-bs-target="#editProfileModal">
        <i class="fas fa-user-pen me-1"></i> UPDATE DATA
      </button>
    </div>
    
    <div class="about-content terminal-bio-well">
      <p id="userBio" class="bio-text-forced"><?php echo nl2br(htmlspecialchars($currentUser['bio'] ?? 'No data logs found. Update profile matrix to generate signature.')); ?></p>
      <a href="#" class="read-more-link" id="readMoreBtn">[ EXPAND TRANSMISSION ]</a>
    </div>
    
    <div class="profile-completeness mt-4 terminal-matrix-status">
      <?php 
      // Calculate profile completeness matrix
      $completeness = 20; // Base 20% validation layer
      ...
      $completeness = min($completeness, 100); // Caps matrix calculation threshold at 100%
      ?>
      
      <div class="d-flex justify-content-between mb-2 layout-metric-titles">
        <span class="matrix-label">INITIALIZATION MATRIX</span>
        <span class="matrix-percentage-value text-purple-data"><?php echo $completeness; ?>% INTEGRATED</span>
      </div>
      
      <div class="progress terminal-linear-bar-frame">
        <div class="progress-bar terminal-linear-bar-fill-purple" role="progressbar" 
      ...
      <div class="form-text mt-2 system-warning-text">
        <i class="fas fa-triangle-exclamation text-amber-warn"></i> 100% sync requested to prevent sub-system authorization latency errors.
      </div>
    </div>
                ...
                <!-- Recent Activity -->
                <div class="card mb-4 gaming-activity-panel border-matrix">
  <div class="card-body">
    
    <h5 class="card-title mb-4 gaming-panel-title">
      <i class="fas fa-wave-square icon-matrix"></i> LIVE EVENT STREAM
    </h5>
    
    <div class="activity-feed terminal-activity-stream">
      
      <div class="activity-item d-flex align-items-start stream-row">
        <div class="activity-icon slot-icon icon-glow-cyan p-2 me-3">
          <i class="fas fa-trophy text-cyan"></i>
        </div>
        <div class="stream-content">
          <div class="stream-header-text">Completed Python Basics</div>
          <div class="stream-timestamp-text">>> 2 HOURS AGO</div>
        </div>
      </div>
      
      <div class="activity-item d-flex align-items-start stream-row">
        <div class="activity-icon slot-icon icon-glow-matrix p-2 me-3">
          <i class="fas fa-flag-checkered text-matrix"></i>
        </div>
        <div class="stream-content">
          <div class="stream-header-text">Challenge Completed: Web Security</div>
          <div class="stream-timestamp-text">>> 1 DAY AGO</div>
        </div>
      </div>
      
      <div class="activity-item d-flex align-items-start stream-row">
        <div class="activity-icon slot-icon icon-glow-purple p-2 me-3">
          <i class="fas fa-book text-purple"></i>
        </div>
        <div class="stream-content">
          <div class="stream-header-text">Started new tutorial: JavaScript ES6+</div>
          <div class="stream-timestamp-text">>> 3 DAYS AGO</div>
        </div>
      </div>
            ...
            <div class="col-lg-4">
                <!-- Connect With Me -->
<div class="card mb-4 gaming-social-panel border-cyan">
    <div class="card-header d-flex justify-content-between align-items-center terminal-social-header">
        <div class="d-flex align-items-center">
            <h5 class="mb-0 gaming-panel-title me-3">
                <i class="fas fa-network-wired icon-cyan"></i> CONNECT FREQUENCIES
            </h5>
            <div id="socialMediaMessage" class="terminal-alert-success" style="display: none;">
                <i class="fas fa-check-circle me-1"></i>
                <span id="socialMediaMessageText" class="alert-matrix-text"></span>
            </div>
        </div>
        <button type="button" class="btn btn-sm btn-gaming-edit" data-bs-toggle="modal" data-bs-target="#socialMediaModal">
            <i class="fas fa-user-gear me-1"></i> MODULATE
        </button>
    </div>
    <div class="card-body terminal-social-body">
        <div class="social-links-grid">
            ...
            <a href="<?php echo !empty($currentUser['social_instagram']) ? 'https://instagram.com/' . htmlspecialchars($currentUser['social_instagram']) : '#'; ?>" 
               class="social-link-node node-instagram" target="_blank">
                <i class="fab fa-instagram node-icon"></i> 
                <span class="node-data-text">
                    <?php if (!empty($currentUser['social_instagram'])): ?>
                        <?php echo htmlspecialchars($currentUser['social_instagram']); ?>
                    <?php else: ?>
                        <span class="node-unset-label">OFFLINE</span>
                    <?php endif; ?>
                </span>
            </a>

            <a href="<?php echo !empty($currentUser['social_facebook']) ? 'https://facebook.com/' . htmlspecialchars($currentUser['social_facebook']) : '#'; ?>" 
               class="social-link-node node-facebook" target="_blank">
                <i class="fab fa-facebook node-icon"></i> 
                <span class="node-data-text">
            ...
            <a href="<?php echo !empty($currentUser['social_twitter']) ? 'https://twitter.com/' . htmlspecialchars($currentUser['social_twitter']) : '#'; ?>" 
               class="social-link-node node-twitter" target="_blank">
                <i class="fab fa-twitter node-icon"></i> 
                <span class="node-data-text">
            ...
            <a href="<?php echo !empty($currentUser['social_pinterest']) ? 'https://pinterest.com/' . htmlspecialchars($currentUser['social_pinterest']) : '#'; ?>" 
               class="social-link-node node-pinterest" target="_blank">
                <i class="fab fa-pinterest node-icon"></i> 
                <span class="node-data-text">
                ...
                <!-- Danger Zone -->
                <div class="card mb-4 gaming-danger-panel border-danger-neon">
    <div class="card-header d-flex align-items-center terminal-danger-header">
        <i class="fas fa-radiation icon-danger-neon me-2"></i> 
        <h5 class="mb-0 gaming-panel-title">CRITICAL OVERRIDES</h5>
    </div>
    <div class="card-body terminal-danger-body">
        <p class="card-text danger-warning-text mb-4">
            [WARNING] THE FOLLOWING PROCEDURES ARE IRREVERSIBLE. AUTHORIZATION LOSS WILL OCCUR IMMEDIATELY UPON EXECUTION.
        </p>
        <div class="d-grid gap-3">
            <button class="btn btn-danger-action" id="btnDeactivate">
                <i class="fas fa-user-slash me-2"></i>DEACTIVATE_OPERATIVE
            </button>
            <button class="btn btn-danger-destructive" id="btnDelete">
                <i class="fas fa-skull-crossbones me-2"></i>PURGE_DATACORE_ENTIRELY
            </button>
        </div>
    </div>
</div>
